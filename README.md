...
To address these challenges, the system introduces the following features:
  ...
  2. Seat booking system
    Passenger enters their details and selects a route they're using. The system then checks if there are any available seats on the minibus and displays them in which the user then chooses which seat they prefer if it is a bus. Therefore they can reserve seats in advance.
